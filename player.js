class Character extends Sprite {
    ...
    handleAttack() {
        if (this.dead) return; // Prevent attacking if the player is dead
    ...
    registerDamage(damage) {
        ...
        if (this.health <= 0) {
            this.dead = true; // Player is dead
            document.querySelector('.alertBoxLose').classList.add('visible');
        }
    }
    ...
    update() {
        if (this.dead) {
            ...
            console.log("Player is dead. No more actions.");
            return;
        }
